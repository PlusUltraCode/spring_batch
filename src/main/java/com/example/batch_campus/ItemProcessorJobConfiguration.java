...
public class ItemProcessorJobConfiguration {
    ...
    public Step step(
        ...
        List<ItemProcessor<User, User>> list = Arrays.asList(processor1(), processor2(), processor3(), processor4());
    ...
    private  ItemProcessor<User, String> customProcessor() {
        return user -> "%s 나이는 %s입니다. 사는 곳은 %s, 전화번호는 %s입니다."
                .formatted(user.getName(), user.getAge(), user.getRegion(), user.getTelephone());
    }

    private  ItemProcessor<User, User> processor1(){
        return user ->{
            user.setName(user.getName()+1234);
            return user;
        };
    }
    ...
    private static ItemProcessor<User, User> processor4(){
        return user ->{
            user.setTelephone(user.getTelephone()+user.getTelephone());
            return user;
        };
    }
